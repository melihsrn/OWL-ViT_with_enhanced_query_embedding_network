...
class HungarianMatcher(torch.nn.Module):
    ...
    def forward(self, pred_logits, pred_boxes,labels,boxes):
        ...
        out_prob = (
            pred_logits.flatten(0, 1).softmax(-1)
        ...
        target_classes_o = torch.cat(
            [t[J] for t, (_, J) in zip([labels], indices)]
        )
        ...
        for n,i in enumerate(idx[0]):
          for nn,j in enumerate(idx[1]):
            if n==nn:
              target_classes[i,j] = target_classes_o[n]
